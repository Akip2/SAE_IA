...
public class Main {
    public static void main(String[] args) {
        try {
            ...
            Donnees donneesTrainFasion = MNISTLoader.loadData("images/Fashion_MNIST/train-images-idx3-ubyte", "images/Fashion_MNIST/train-labels-idx1-ubyte", 1000);
            ...
            // Fashion MNIST
            int nbPixelsImagette = donneesTrainFasion.getImagette(0).getFlatPixels().length;
            ...
            AlgoMLP algoMLP = new AlgoMLP(donneesTrainFasion, layers, tauxApprentissage, fonctionActivation2);

            System.out.println("START : Précision train : " + Statistique.calculerPrecision(algoMLP, donneesTrainFasion) + " ; Précision test : " + Statistique.calculerPrecision(algoMLP, donneesTestFasion));

            double erreur_cible = 0.001;
            int max_iterations = 10;
            for (int i = 0; i < 10; i++) {
                double erreur = algoMLP.train(erreur_cible, max_iterations);
                System.out.println("Itération " + i + " Erreur : " + erreur + " Précision train : " + Statistique.calculerPrecision(algoMLP, donneesTrainFasion) + " ; Précision test : " + Statistique.calculerPrecision(algoMLP, donneesTestFasion));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
