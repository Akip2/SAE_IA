...
public class MLP extends AlgoClassification{
    ...
    public MLP(Donnees donnees, int[] layers, double learningRate, TransferFunction fun) {
        super(donnees);
    ...
    public int predireEtiquette(Imagette imagette) {
        int maxIndex = 0;
        System.out.println("input : " + Arrays.toString(imagette.getFlatPixels()));
        double[] output = this.execute(imagette.getFlatPixels());
    ...
    public double train(double erreur_cible, int max_iterations) {
        ...
        while ((erreur_courante > erreur_cible) && iteration < max_iterations) {
            ...
            for (int indice = 0; indice < totalImagettes; indice++) {
                double[] labelOneHot = new double[this.getOutputLayerSize()];
                int label = donneesEntrainement.getImagette(indice).getLabel();
                labelOneHot[label] = 1.0;

                double erreur = this.backPropagate(
